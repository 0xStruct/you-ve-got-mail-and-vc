...
const vDevpostSubmissionRouter = express.Router();

vDevpostSubmissionRouter.get("/", (req, res) => {
    res.send("route: vDevpostSubmissionRouter");
});

vDevpostSubmissionRouter.post("/", async (req, res) => {
    ...
    while (verifyFailed === false) {
        ...
        // check subject
        if (!subject.startsWith("Submission confirmed:")) {
            verifyFailed = true;
            verifyFailedMessage = "Invalid Subject field";
            break;
        }

        var projectName = subject.split("Submission confirmed: ")[1];
        ...
        const issueResponse = await trinsic.credential().issueFromTemplate({
            // required
            templateId: "https://schema.trinsic.cloud/stupefied-haibt-zg8jgredzmdh/proof-by-email",
...
export default vDevpostSubmissionRouter;
