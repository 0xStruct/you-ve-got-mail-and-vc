...
const vTrinsicEmailRouter = express.Router();

vTrinsicEmailRouter.get("/", (req, res) => {
    res.send("route: vTrinsicEmailRouter");
});

vTrinsicEmailRouter.post("/", async (req, res) => {
    ...
    while (verifyFailed === false) {
        ...
        // check subject
        if (!subject.startsWith("E-book download:")) {
            verifyFailed = true;
            verifyFailedMessage = "Invalid Subject field";
            break;
        }

        var bookName = subject.split("E-book download: ")[1];
...
export default vTrinsicEmailRouter;
